...
final class MemcachedEnhancer {
    ...
    static void enhance(MetaClass mc, MemcachedProvider provider = MemcachedClientHolder.instance) {
        ...
        mc.withMemcached = {Closure closure ->
            provider.withMemcached('default', closure)
        }
        mc.withMemcached << {String clientName, Closure closure ->
            provider.withMemcached(clientName, closure)
        }
        mc.withMemcached << {CallableWithArgs callable ->
            provider.withMemcached('default', callable)
        }
        mc.withMemcached << {String clientName, CallableWithArgs callable ->
            provider.withMemcached(clientName, callable)
        }
    }
}
